...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname |tic tac toe starter (2)|) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
(require 2htdp/image)
    ...
    [(define (terminal? board) (< (board-state 4)))
     (define (min-max board)
       (local [(define possible-moves (
                                       ...
                                       (fill-square (min-max board)))
                 (fill-square board (min-max board 2)))]))]))

#;(define (next-move board)
    fill-square (minimax board))

;;board integer player --> board
;; -board is the current board
;; -depth is the move that computer need to look ahead
;; -maximizingplayer is player who plays as max

#;
(define minimax (board depth maximizingplayer)
  (cond [(= depth 0) board]
        [(if (= current-player maximizingplayer)
             (apply max (minimax (lob (- depth 1) next-player))))]
        [else
         (apply min (minimax (lob (- depth 1) next-player)))]
        )
  )

;;board-->listOfboard
;; find a legal move and insert with X
(define (next-board ws)
...
(define (random-move board)
  ...
  (if (= false (player-num (list-ref board pos)))
      (fill-square board pos P2)
      (random-move board)
      )
  )
;; WS player --> boolean
;; abstract function to deternine winning --> if one of the units has all same value --> win
(define (WIN ws player)
  (ormap (lambda(i) (andmap (lambda(k) (equal? player (list-ref (ws-board ws) k))) i)) UNITS))
...
(define (check-empty board)
  (cond [(empty? board) false]
        [else
         (if(not (equal? false (player-num (first board))))
           (check-empty (rest board))
            false)]))
...
(define (board-state board)
  (cond
        [(WIN board P1) -1]
        [(WIN board P2) 1]
        [(equal? check-empty false) 0] ;;> when there is no empty space for legal move --> board is full
        [else 5]))
...
(define (tock ws)
  (if (equal? P2 (ws-player ws))
      (make-ws (next-board (ws-board ws)) P2)
      ws))
